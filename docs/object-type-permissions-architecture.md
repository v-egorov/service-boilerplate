...
When a SINGLE role grants multiple scoped variants for the same action, the most specific available variant is used first. If broader variants exist in the union from other roles, those override.

```
User has: portfolio:read:own (from "user" role)
Request: GET /portfolio/123 (owned by user) → allowed (:own check passes)
         GET /portfolio/456 (owned by someone else) → denied (:own check fails, no :all in union)

User has: portfolio:read:own (from "user") + portfolio:read:all (from "admin")  
Request: GET /portfolio/456 (owned by someone else) → allowed (:all from admin role overrides :own requirement)
```
...
| Permission resolution (within-role) | Most specific variant in union set takes effect |

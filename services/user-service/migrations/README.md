...
This directory contains database migrations for the user-service service.
...
├── development/              # Development migrations (6 files)
├── staging/                  # Staging migrations (4 files)
├── production/               # Production migrations (3 files)
...
- **Development**: 6 migrations (includes test data: users, test passwords)
- **Staging**: 4 migrations (excludes dev-only test data)
- **Production**: 3 migrations (excludes dev-only test data)

## Schema Information

This service uses the `user_service` database schema. All tables are created within this schema to ensure proper isolation between services.

## Migration Guidelines

1. Each migration should be numbered sequentially within its environment
2. Include both up and down migrations
3. Always use schema-qualified table names (e.g., `user_service.table_name`)

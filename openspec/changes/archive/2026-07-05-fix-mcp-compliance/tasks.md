...
- [x] 1.1 Create `ListObjectTypesResult` struct in type_tools.go: `{Types []ObjectType}` with json:"types" tag (domain-specific key)
- [x] 1.2 Create `GetObjectTypeResult` struct in type_tools.go: single ObjectType (no wrapper needed for single object)
- [x] 1.3 Create `ListObjectsResult` struct in object_tools.go: `{Objects []Object}` with json:"objects" tag (domain-specific key)
- [x] 1.4 Create `GetObjectResult` struct in object_tools.go: single Object (no wrapper needed)

## 2. Add output schemas to tool registrations

- [x] 2.1 In type_tools.go: add `mcp.WithOutputSchema[ListObjectTypesResult]()` to list_object_types tool definition
- [x] 2.2 In type_tools.go: add `mcp.WithOutputSchema[GetObjectTypeResult]()` to get_object_type tool definition
- [x] 2.3 In object_tools.go: add `mcp.WithOutputSchema[ListObjectsResult]()` to list_objects tool definition
- [x] 2.4 In object_tools.go: add `mcp.WithOutputSchema[GetObjectResult]()` to get_object tool definition

## 3. Fix structuredContent serialization for array tools

- [x] 3.1 In type_tools.go — list_object_types handler: wrap result as `ListObjectTypesResult{Items: types}` before assigning StructuredContent; replace full-data JSON in Content with summary string
- [x] 3.2 In object_tools.go — list_objects handler: wrap result as `ListObjectsResult{Items: objects}` before assigning StructuredContent; replace full-data JSON in Content with summary string
...
- [x] 4.1 Add `ParentTypeID *int64` field to `ListObjectTypesParams` struct in type_tools.go
...
- [x] 5.1 In object_tools.go: add `TypeKeyPrefix *string` field to `ListObjectsParams` struct

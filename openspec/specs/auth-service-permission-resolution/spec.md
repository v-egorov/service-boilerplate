# auth-service-permission-resolution Specification

## Purpose
Defines the permission resolution behavior in auth-service's `hasPermission()` function — specifically how database permission entries with empty scope (unscoped/flat permissions) are matched against scoped requirement strings. Establishes that an unscoped DB entry is unrestricted and satisfies any required scope level (`:all`, `:own`, or none).
## Requirements

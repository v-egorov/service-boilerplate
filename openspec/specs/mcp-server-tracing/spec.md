# mcp-server-tracing Specification

## Purpose
Add OpenTelemetry distributed tracing to the MCP server so that every inbound request (tools, resources, prompts) produces a visible span in Grafana/Jaeger, trace context propagates correctly to downstream services (objects-service), and business-level spans provide full observability across the entire call chain.
## Requirements
